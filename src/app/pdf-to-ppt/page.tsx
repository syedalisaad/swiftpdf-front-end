  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (extension === "pdf") {
        ...
        toast.error("Invalid File", { description: "Please select a valid PDF file." });
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] py-12 px-4 font-sans">
      <div className="max-w-4xl mx-auto">
        <Link href="/" className="inline-flex items-center text-gray-500 hover:text-rose-600 mb-8 transition-colors group">
          ...
          <div className="p-6 md:p-10">
                ...
                {!file ? (
                  <div className="relative group">
                    ...
                    >
                      ...
                      <p className="text-gray-400 font-medium">Click to upload .pdf</p>
                    </label>
                  </div>
                ) : (
                  <div className="space-y-8">
                    <FileCard file={file} index={0} file_type="pdf" onRemove={() => setFile(null)} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-rose-50/50 p-4 rounded-2xl border border-rose-100 flex items-center gap-3">
                            ...
                            <p className="text-xs font-semibold text-rose-800">High-Resolution Slides</p>
                        </div>
                        <div className="bg-rose-50/50 p-4 rounded-2xl border border-rose-100 flex items-center gap-3">
                            <ShieldCheck className="text-rose-600" size={20} />
                            <p className="text-xs font-semibold text-rose-800">Encrypted Transfer</p>
                        </div>
                    </div>
            ...
            {status === "processing" && (
              <div className="py-20 text-center space-y-6">
                <div className="relative w-24 h-24 mx-auto">
                    <Loader2 className="animate-spin text-rose-500 absolute inset-0" size={96} strokeWidth={1} />
                    <div className="absolute inset-0 flex items-center justify-center">
                        <FileText className="text-rose-200" size={32} />
                    </div>
                </div>
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">Generating Slides...</h2>
                    <p className="text-gray-400 mt-2 font-medium italic">Processing pages into presentation format.</p>
                </div>
              </div>
            )}
            ...
            {status === "success" && downloadUrl && (
              <div className="py-10 text-center animate-in zoom-in-95 duration-500">
                ...
                <h2 className="text-3xl font-black text-gray-800 mb-2">PowerPoint Ready</h2>
                <p className="text-gray-500 mb-10 max-w-xs mx-auto">Your PDF has been transformed into an editable slide deck.</p>
